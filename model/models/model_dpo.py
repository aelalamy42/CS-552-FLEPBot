...
class AutoDPOModelForCausalLM(PreTrainedModelWrapper):
    ...
    def __init__(self, pretrained_model, **kwargs):
        ...
        self.custom_module = AutoModelForCausalLM.from_config(self.pretrained_model.config, **custom_module_kwargs, trust_remote_code=True)
        self.device = 'cuda:0'
    ...
    def post_init(self, state_dict):
        ...
        IMPORTANT: Make sure to replace `custom_module` with the name of your custom module class name.
        """
        if not hasattr(self, 'open_elm'):
            return
        
        for k in list(state_dict.keys()):
            if "open_elm." in k:
                state_dict[k.replace("open_elm.", "")] = state_dict.pop(k)
    ...
    def forward(
        ...
        **kwargs,
        ...
        output_dict = self.custom_module(input_ids=input_ids, attention_mask=attention_mask, **kwargs)
    ...
    def get_logprobs(self, batch, tokenizer):
        ...
        # Combine prompts with chosen and rejected responses
        combined_chosen = [prompt + chosen for prompt, chosen in zip(prompt_batch, chosen_batch)]
        combined_rejected = [prompt + rejected for prompt, rejected in zip(prompt_batch, rejected_batch)]

        # Tokenize the combined inputs in batch
        chosen_encoding = tokenizer(combined_chosen, padding=True, truncation=True, return_tensors="pt").to(self.device)
        rejected_encoding = tokenizer(combined_rejected, padding=True, truncation=True, return_tensors="pt").to(self.device)

        chosen_encoding.to(device=self.device)
        rejected_encoding.to(device=self.device)
        
        self.custom_module.to(device=self.device)
        # Perform model inference in batch
        with torch.no_grad():
            chosen_output = self.custom_module(input_ids=chosen_encoding["input_ids"], attention_mask=chosen_encoding["attention_mask"])
            rejected_output = self.custom_module(input_ids=rejected_encoding["input_ids"], attention_mask=rejected_encoding["attention_mask"])

        # Get the target tokens
        chosen_target = chosen_encoding["input_ids"].clone()
        chosen_target = chosen_target != tokenizer.pad_token_id

        rejected_target = rejected_encoding["input_ids"].clone()
        rejected_target = rejected_target != tokenizer.pad_token_id

        # Target tokens must be dtype=int64
        chosen_target = chosen_target.to(torch.int64)
        rejected_target = rejected_target.to(torch.int64)

        # Compute log probabilities for the last token in each sequence
        chosen_log_prob = F.log_softmax(chosen_output.logits, dim=-1)
        rejected_log_prob = F.log_softmax(rejected_output.logits, dim=-1)

        chosen_logps = chosen_log_prob.gather(2, index=chosen_target.unsqueeze(2)).squeeze(2)
        rejected_logps = rejected_log_prob.gather(2, index=rejected_target.unsqueeze(2)).squeeze(2)

        return chosen_logps.sum(dim=-1), rejected_logps.sum(dim=-1)
    ...
    def prediction_step_mcqa(self, batch, tokenizer):
        ...
        self.custom_module = self.custom_module.to(self.device)
        
        # Encode all questions for the prompts
        questions_encoding = tokenizer(batch["question"], padding=True, truncation=True, return_tensors="pt", max_length=1000).to(device=self.device)
        
        # Generate answers to each prompts
        with torch.no_grad():
            output = self.custom_module.generate(**questions_encoding, max_new_tokens=20, eos_token_id = tokenizer.eos_token_id, pad_token_id = tokenizer.eos_token_id)
        
        # Convert the prediction to letters using the tokenizer
        preds = tokenizer.batch_decode(output)
        
        # Function to select the answer, count the number of occurence of one of the possible answers
        # and select the one with the highest occurence, when there is a tie or no occurence, select randomly
        def select_answer(output, prompt):
            output = output.replace(prompt, "")
            counts = {c: output.count(c) for c in ['A', 'B', 'C', 'D']}

            if all(v == 0 for v in counts.values()):
                return random.choice(['A', 'B', 'C', 'D'])
            elif list(counts.values()).count(max(counts.values())) > 1:
                return random.choice([k for k, v in counts.items() if v == max(counts.values())])
            return max(counts, key=counts.get)
        
        for q, p in zip(batch['question'], preds):
            output_dict['preds'].append(select_answer(p, q))           
        return output_dict
